...
if (!isset($_SESSION['logged_in'])) {
    ...
    if (isset($_POST['place_order'])) {
        ...
        foreach ($_SESSION['cart'] as $key => $value) {
            ...
            $stmt1 = $conn->prepare("INSERT INTO order_items( order_id,product_id, product_name, product_image,user_id,order_date,product_price)
         VALUES(?,?,?,?,?,?,?)");

            $stmt1->bind_param('iississ', $order_id, $product_id, $product_name, $product_image, $user_id, $order_date, $product_price);
            $stmt1->execute();
        }
